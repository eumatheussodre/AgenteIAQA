...
from g_Test_Stress import salvar_csv, salvar_json
import requests
import json
...
if menu == "Gerar Casos de Teste":
    ...
    st.markdown("Faça upload dos documentos necessários para gerar os casos de teste.")
...
# 📊 Gerador de Massa de Dados para QA
    ...
    st.markdown("Defina os campos, status e gênero para gerar dados sintéticos.")
    ...
    status_opcoes = st.multiselect("Status possíveis", ["ativo", "inativo", "pendente", "bloqueado"], default=["ativo", "inativo", "pendente", "bloqueado"])
    generos_opcoes = st.multiselect("Gêneros possíveis", ["Masculino", "Feminino", "Outro"], default=["Masculino", "Feminino", "Outro"])

    if st.button("📊 Gerar Dados"):
        massa_dados = gerar_massa_de_dados(
            quantidade=quantidade,
            status_opcoes=status_opcoes,
            generos_opcoes=generos_opcoes,
            campos=campos
        )
